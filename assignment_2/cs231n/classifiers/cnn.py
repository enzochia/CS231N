...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        cache_dict = {}
        # N, C, H, W = X.shape
        X, cache_dict[1] = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
        # _, F, H_prime, W_prime = X.shape
        X, cache_dict[2] = affine_relu_forward(X, W2, b2)
        scores, cache_dict[3] = affine_forward(X, W3, b3)
        ...
        dx, dw, db = affine_backward(dx, cache_dict[3])  
        grads["W3"] = dw + self.reg * self.params["W3"]  
        grads["b3"] = db
        dx, dw, db = affine_relu_backward(dx, cache_dict[2])
        grads["W2"] = dw + self.reg * self.params["W2"]  
        grads["b2"] = db
        dx, dw, db = conv_relu_pool_backward(dx, cache_dict[1])

...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = bce_loss(logits_real, torch.ones_like(logits_real).type(dtype).squeeze())
    loss += bce_loss(logits_fake, torch.zeros_like(logits_fake).type(dtype).squeeze())
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = torch.sum((scores_real - 1) ** 2) / (scores_real.shape[0] * 2)
    loss += torch.sum((scores_fake) ** 2) / (scores_fake.shape[0] * 2)
...
def ls_generator_loss(scores_fake):
    ...
    loss = torch.sum((scores_fake - 1) ** 2) / (scores_fake.shape[0] * 2)

...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        row = torch.arange(0, max_len).float().unsqueeze(1)
        exp_term = torch.exp(torch.arange(0, embed_dim, 2).float() * 
                              (-math.log(10000.0) / embed_dim))
        pe[0, :, 0::2] = torch.sin(row * exp_term)
        pe[0, :, 1::2] = torch.cos(row * exp_term)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        attn = torch.matmul(q, k.transpose(-2, -1)) * (1.0 / math.sqrt(self.head_dim))
        if attn_mask is not None:
          attn = attn.masked_fill(attn_mask[None, None, :, :] == 0, float('-inf'))
        attn = F.softmax(attn, dim=-1)
        attn = self.attn_drop(attn)
        y = torch.matmul(attn, v).transpose(1, 2).contiguous().view(N, S, E)
        output = self.proj(y)

...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        encode_captions = self.embedding(captions)
        encode_captions = self.positional_encoding(encode_captions)
        tgt_mask = torch.tril(torch.ones(T, T))
        encode_img = self.visual_projection(features.unsqueeze(1))
        y = self.transformer(encode_captions, encode_img, tgt_mask)

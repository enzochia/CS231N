...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.matmul(z_i, z_j.unsqueeze(1)) / \
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += - torch.log((torch.exp(torch.matmul(z_k, z_k_N.unsqueeze(1)) /
                                             (torch.linalg.norm(z_k) * torch.linalg.norm(z_k_N) * tau))) / 
                                  (torch.sum(torch.exp(torch.matmul(out, z_k.unsqueeze(1)).squeeze() / 
                                              (torch.linalg.norm(out, dim=1) * torch.linalg.norm(z_k)
                                               * tau))) - torch.exp(torch.tensor(1.0 / tau))))
                                    
        total_loss += - torch.log((torch.exp(torch.matmul(z_k_N, z_k.unsqueeze(1)) /
                                             (torch.linalg.norm(z_k_N) * torch.linalg.norm(z_k) * tau))) / 
                                  (torch.sum(torch.exp(torch.matmul(out, z_k_N.unsqueeze(1)).squeeze() / 
                                              (torch.linalg.norm(out, dim=1) * torch.linalg.norm(z_k_N)
                                               * tau))) - torch.exp(torch.tensor(1.0 / tau))))                                               
...
def sim_positive_pairs(out_left, out_right):
    ...
    out_left /= torch.linalg.norm(out_left, dim=1).unsqueeze(1)
    out_right /= torch.linalg.norm(out_right, dim=1).unsqueeze(1)
    pos_pairs = torch.sum(out_left * out_right, dim=1).unsqueeze(1)
...
def compute_sim_matrix(out):
    ...
    norm_vec = torch.linalg.norm(out, dim=1).unsqueeze(1)
    sim_matrix = torch.matmul(out, out.T) / torch.matmul(norm_vec, norm_vec.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos_pairs = torch.diagonal(sim_matrix, N)
    sim_pos_pairs = torch.cat([sim_pos_pairs, sim_pos_pairs], dim=0).unsqueeze(1)
